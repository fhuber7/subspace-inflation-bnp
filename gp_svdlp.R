gp_bnp <- function(y, X, yho, Xho,nburn=5000, ntot=10000, thinning=1, PDP=FALSE, sample.omega=TRUE, tau2.start=0.5, G.max=25, fcst=TRUE, a0=1, a1=1, PCA=TRUE, mix.sv=FALSE){
  ...
  if (G==1) zeta.0 <- 10^5 else zeta.0 <- 10 #change this to 0.01 if CPIAUCSL is forecast
  ...
  for (irep in -(nburn-1):ntot){
    ...
    Dinv <- try(solve(K1 + diag(sd.t^2)), silent=TRUE)
    if (is(Dinv, "try-error")) Dinv <- MASS::ginv(K1 + diag(sd.t^2))
    ...
    #Sample the common shrinkage factor from a GIG distribution
    # if (G.max > 1){
    #   a.j <- 2*c.0
    #   p.k <- c.1 - G/2
    #   b.j <- sum(mu.G^2)
    #   zeta.0 <- 1/GIGrvg::rgig(1, p.k, b.j, a.j) #precision already
    #   
    #   #tau0 <- GIGrvg::rgig(1, o.1 - v*G/2, v*sum(1/sd.G^2), 2*o.2)
    # }
    ...
    for (t in seq_len(n)){
      ...
      prob.non <- (u.i < xi[1:G])/xi[1:G] * eta[1:G] * dnorm(y.hat[[t]], mu.G[1:G], exp(r.t[[t]]/2)*sd.G[1:G]) + 1e-32
    ...
    #Step 6: Sample the hyperparameters of the Kernel matrix
    y.hat <- y-mu.t
    ...
    if (is(Kn.inv.prop, "try-error")) ml.new <- -Inf else{
      ...
      Dinv.prop <- try(solve(K1.prop + diag(sd.t^2)), silent=TRUE)
      if (is(Dinv.prop, "try-error")) Dinv.prop <- MASS::ginv(K1.prop + diag(sd.t^2))
    ...
    if (irep > 0 && irep %% thinning == 0){
      ...
      if (fcst){
        ...
        y.pred <- f.star.draw + rnorm(1, mu.G[[s.star]], exp(r.t[[n]]/2)*sd.G[[s.star]])
        
        
        pred.store[irep/thinning, ] <- y.pred
        fevd.store[irep/thinning, ] <- sqrt(V.f.mean)/(sqrt(V.f.mean) + exp(r.t[[n]]/2)*sd.G[[s.star]])
        lpl.store[irep/thinning, ] <- dnorm(yho[[2]], f.star.draw + mu.G[[s.star]], exp(r.t[[n]]/2)*sd.G[[s.star]], log=TRUE) #second element --> final vintage
      }
      # browser()
