...
// [[Rcpp::export]]
arma::mat GaussKernel(arma::mat X, double h = 1, double sigf = 1){
    int i,j;
    double n = X.n_cols;
    double p = X.n_rows;
    mat K = zeros<mat>(n,n);
    for (i = 0; i<n; i++){
        for(j = 0; j<n; j++){
            if(i==j){
                ...
                K(i,j) = sigf * exp(-h/(p)*sum(pow(X.col(i)-X.col(j),2)));
            }
        }
    }
    return K + trans(K);
...
// [[Rcpp::export]]
arma::mat GaussKernel_star(arma::mat Xstar,arma::mat X, double h = 1, double sigf = 1){
  int i,j;
  double n = Xstar.n_cols;
  double n0 = X.n_cols;
  double p = X.n_rows;
  mat K = zeros<mat>(n,n0);
  for (i = 0; i<n; i++){
    for(j = 0; j<n0; j++){
        K(i,j) = sigf * exp(-h/(p)*sum(pow(Xstar.col(i)-X.col(j),2)));
    }
  }
  return K;
}
